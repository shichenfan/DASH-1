...
pal <- colorBin(
  palette = "RdYlGn",
  na.color = "#808080", 
  bins = c(0,10,20,30,40,50,75),
  domain = NULL) # speed 

paltti <- colorBin(
  palette = "RdYlGn",
  na.color = "#808080",
  bins = c(0,1.1,1.3,1.6,2,2.5,10),
  ...
  observeEvent({input$tti}, {
    if(input$tti == "tti"){
      leafletProxy("map2", data = spdtti) %>%
        ...
        addLegend("bottomright", 
                  pal     = paltti,
                  values  = eval(parse(text = paste0('~tti.AM'))),
                  title   = paste0("INRIX Average AM Travel Time Index 2018"),
                  opacity = 0.8,
                  layerId = 1)
      ...
      leafletProxy("map3", data = spdtti) %>%
        ...
        addLegend("bottomright", 
                  pal     = paltti,
                  values  = eval(parse(text = paste0('~tti.PM'))),
                  title   = paste0("INRIX Average PM Travel Time Index 2018"),
                  opacity = 0.8,
                  layerId = 1)
      }else{
        leafletProxy("map2", data = spdtti) %>%
          ...
          addLegend("bottomright", 
                    pal     = pal, 
                    values  = eval(parse(text = paste0('~speed.AM'))),
        ...
        leafletProxy("map3", data = spdtti) %>%
          ...
          addLegend("bottomright",
                    pal     = pal,
                    values  = eval(parse(text = paste0('~speed.PM'))),
  ...
  output$gif <- renderUI({
    if(input$gif == 'map'){
      ...
      tagList(
        ...
        HTML('<video width = "900" height = "750" controls> <source src="2018tti.mp4" type = "video/mp4"></video>') 
      )
      } else{
      tagList(
        ...
        HTML('<video width = "900" height = "750" controls> <source src="2018speed.mp4"" type = "video/mp4"></video>') 
        )}
    })
